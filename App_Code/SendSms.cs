...
public class SendSms
{
    ...
    public string sendotp(string Vmessage, string Vmobno)
    {
		...
		//HttpWebRequest req = (HttpWebRequest)WebRequest.Create("http://login.arihantsms.com/vendorsms/pushsms.aspx?user=lionsdistrictf2&password=demo12345&msisdn="+Vmobno+"&sid=SANJDG&msg="+Vmessage+"&fl=0&gwid=2");
		
        //HttpWebRequest req = (HttpWebRequest)WebRequest.Create("http://bulksms.dnsitexperts.com/sendsms.jsp?user=" + Username + "&password=" + Password + "&mobiles=" + Vmobno + "&senderid=" + SenderId + "&sms=" + Vmessage + "");

        HttpWebRequest req = (HttpWebRequest)WebRequest.Create("http://promosms.itfuturz.com/vendorsms/pushsms.aspx?user=djeeto&password=dns123&msisdn=" + Vmobno + "&sid=DJEETO&msg=" + Vmessage + "&fl=0&gwid=2");
